...
public class HW2CLI {
	...
	private static final String				FILE_DATASET_USERS		=	"users.dat";
	private static final String				FILE_DATASET_MOVIES		=	"movies.dat";
	private static final String				FILE_DATASET_RATINGS	=	"ratings.dat";
		...
		while (isRunning) {
			...
			try {
				switch (input) {
					...
					case "loadMovies":			hw2API.loadMovies(pathDatasetFolder + FILE_DATASET_MOVIES);		break;
					case "loadUsers":			hw2API.loadUsers(pathDatasetFolder + FILE_DATASET_USERS);		break;
					case "loadRatings":			hw2API.loadRatings(pathDatasetFolder + FILE_DATASET_RATINGS);	break;
					case "movie":				hw2API.movie(Long.parseLong(tokens[1]));						break;
					case "user":				hw2API.user(Long.parseLong(tokens[1]));							break;
					case "userRatings":			hw2API.userRatings(Long.parseLong(tokens[1]));					break;
					case "movieRatings":		hw2API.movieRatings(Long.parseLong(tokens[1]));					break;
	...
	private void printHelp() {
		...
		System.out.println("loadMovies\t\t prase and lode the movies");
		System.out.println("loadUsers\t\t prase and lode the users");
		System.out.println("loadRatings\t\t prase and lode the ratings");
		System.out.println("movie * \t\t print the info for movie *");
		System.out.println("user *  \t\t print the info for user *");
		System.out.println("userRatings *\t\t print the ratings for user *");
		System.out.println("movieRatings *\t\t print the ratings for movie *");
	...
	private void parsePassedFolders(String[] args) throws Exception {
		...
		validateFileExists(pathDatasetFolder, FILE_DATASET_USERS);
		validateFileExists(pathDatasetFolder, FILE_DATASET_MOVIES);
		validateFileExists(pathDatasetFolder, FILE_DATASET_RATINGS);
